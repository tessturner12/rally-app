...
No TfL API key set up yet — TfL's API works fine unauthenticated for now (just rate-limited), and `tfl.ts` still has a TODO marking where KV caching needs to be wired in (not done yet — it works without it, just slower/uncached, and the same is true for venue caching being unverified beyond the unit tests).
