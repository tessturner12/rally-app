...
describe('getJourneys', () => {
  ...
  test('never runs more than 80 journey lookups at the same time', async () => {
    let inFlight = 0
    let maxInFlight = 0
    const fetchMock = vi.fn().mockImplementation(async () => {
      inFlight++
    ...
    const pairs = Array.from({ length: 150 }, () => ({
      fromLat: 51.5,
      fromLng: -0.1,
      toLat: 51.6,
      toLng: -0.2,
    ...
    // Also asserts we actually reach past the old 40-concurrent ceiling,
    // not just that we stay under the new one.
    expect(maxInFlight).toBeGreaterThan(40)
    expect(maxInFlight).toBeLessThanOrEqual(80)
  })
})
